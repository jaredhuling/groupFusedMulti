#ifndef ADMMOGLASSOWIDE_H
#define ADMMOGLASSOWIDE_H
...
// f(x) => 1/2 * ||Ax - b||^2
// g(z) => lambda * ||z||_1
class ADMMogLassoWide: public FADMMBase<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd>
{
protected:
    ...
    void next_beta(Vector &res)
    {
        ...
        Vector rhs = ( datX * (rhs2.array() * CC_inv.array().square()).matrix() );
...
public:
    ...
    void init(double lambda_, double rho_)
    {
        ...
        if(rho <= 0)
        {
            ...
            Spectra::SymEigsSolver<Spectra::DenseSymMatProd<double>> eigs(op, 2, 5);
        ...
        MatrixXd matToSolve(XWXt(datX, CC_inv));
        matToSolve.array() /= rho;
        matToSolve.diagonal().array() += one;

        // precompute LLT decomposition of (I + (1/rho) * X * (D'D)^{-1} X^T )
    ...
    void init_warm(double lambda_)
    {
        ...
        if (rho_unspecified)
        {
            ...
            MatrixXd matToSolve(XWXt(datX, (CC_inv.array() / rho).matrix()));
            matToSolve.diagonal().array() += one;
