...
RcppExport SEXP admm_oglasso_fused_dense_lammat(SEXP x_,
                                         SEXP y_,
                                         SEXP delta_,
                                         SEXP group_,
                                         SEXP fused_,
                                         ...
                                         SEXP opts_)
{
    ...
    MatrixXd lambda(as<MatrixXd>(lambda_));
    int nlambda = lambda.rows();
    ...
    const double use_alpha_param  = as<double>(opts["use.alpha.param"]);
    ...
    int nlam_tmp        = as<int>(nlambda_);
    
    // list to store beta matrices
    Rcpp::List beta_list(nlambda_fused);
    ...
    if(nlambda < 1)
    {
        ...
        lambda.resize(nlam_tmp, nlambda_fused);
        if (use_alpha_param)
        {
            
            for (int i = 0; i < lambda.cols(); ++i)
            {
                double lmax_use = lmax / lambda_fused(i);
                double lmin = as<double>(lambda_min_ratio_) * lmax_use;
                
                lambda.col(i).setLinSpaced(as<int>(nlambda_), std::log(lmax_use), std::log(lmin));
            }
        } else
        {
            ...
            for (int i=0; i < lambda.cols(); ++i)
            {
                lambda.col(i).setLinSpaced(as<int>(nlambda_), std::log(lmax), std::log(lmin));
            }
        }
        lambda = lambda.array().exp();
        nlambda = lambda.rows();
    }
    ...
    bool tall_condition = n > 2 * p || p < 4500;
    
    double maxbetaval = 0.0;

    if (adaptive_lasso || adaptive_fused)
    {
        VectorXd d, fu;
        //ilambda = 1e-10 * lambda[0] * n / datstd.get_scaleY();
        ilambda = 20e-1 * lambda(nlambda - 1, nlambda_fused - 1) * n / datstd.get_scaleY();
        if(tall_condition)
        {
            if (p <= n)
            {
                ...
                maxbetaval = beta_unpen.array().abs().maxCoeff();

                // if the MLEs are divergent then use
                // a linear probability model
                
                int ct = 0;
                while ((!std::isfinite(maxbetaval) || maxbetaval > 15) && ct < 20)
                {
                    ct += 1;
                    ...
                    double ilam2 = ilambda * std::pow(1.5, ct);
                    solver_tall_2->init(ilam2, rho, ilam2);
        ...
        if (!tall_condition || (!std::isfinite(maxbetaval) || maxbetaval > 15))
        {
        ...
        if (adaptive_fused)
        {
            for (int ff = 0; ff < nfused; ++ff)
            {
                double abs_diff = std::pow( std::abs(fu(ff)), gamma );
                if (abs_diff < 1e-5)
                {
                    abs_diff = 1e-5;
    ...
    for (int j = 0; j < nlambda_fused; j++)
    {
        ...
        {
            if (use_alpha_param) // in this case, lambda_fused is a mixing param 0 < lambda_fused < 1
            { // where lambda_fused = 1 indicates NO fused lasso and lambda_fused = 0 is ALL fused lasso
                ilambdaF = (1.0 - alpha_cur) * lambda(i,j) * double(n) / datstd.get_scaleY();
                ilambda  =        alpha_cur  * lambda(i,j) * double(n) / datstd.get_scaleY();
            } else
            {
                ilambda  = lambda(i,j) * double(n) / datstd.get_scaleY();
            }
            ...
            if(n > 2 * p || p < 4500)
            {
                ...
                {
                    // get associated loss
                    loss_tmp(i) = solver_tall->get_loss();
    ...
    return List::create(Named("lambda")                 = lambda,
                        ...
                        Named("lambda.fused")           = lambda_fused,
                        Named("nlambda.fused")          = nlambda_fused,
