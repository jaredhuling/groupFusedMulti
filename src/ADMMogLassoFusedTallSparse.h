#ifndef ADMMOGLASSOFUSEDTALLSPARSE_H
#define ADMMOGLASSOFUSEDTALLSPARSE_H
...
template <class T>
T sparseMax (Eigen::SparseMatrix<T>& mat) { 
    return Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1> >(mat.valuePtr(), mat.nonZeros()).maxCoeff();
}
...
// f(x) => 1/2 * ||Ax - b||^2
// g(z) => lambda * ||z||_1
class ADMMogLassoFusedTallSparse: public FADMMBaseTwo<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd>
{
protected:
    ...
    const MSpMat  datX;                  // sparse data matrix
                                      ...
                                      const double &lam, const double &step_size)
    {
        ...
        for (int g = 0; g < ngroups; ++g)
        {
            ...
            double thresh_factor = std::max(0.0, 1.0 - step_size * lam * group_weights(g) / (ds_norm) );
...
public:
    ADMMogLassoFusedTallSparse(const MSpMat &datX_, //ConstGenericMatrix &datX_,
    ...
    FADMMBaseTwo<Eigen::VectorXd, Eigen::VectorXd, Eigen::VectorXd>
             (datX_.cols(), C_.rows(), D_.rows(), C_.rows() + D_.rows(),
              ...
              lambda0(XY.segment(intercept_, nvars - intercept_).cwiseAbs().maxCoeff())
    { }
    ...
    void init(double lambda_, double rho_, double lambdaF_)
    {
        ...
        if(rho <= 0)
        {
            ...
            SparseSymMatProd<Double> op(XX);
            //Spectra::SymEigsSolver< Double, Spectra::LARGEST_ALGE, MatOpSymLower<Double> > eigs(&op, 1, 3);
            Spectra::SymEigsSolver<SparseSymMatProd<double>> eigs(op, 3, 6);
            
            // Initialize and compute
            eigs.init();
            int nconv = eigs.compute(SortRule::BothEnds);
            ...
            // std::cout << "eigs: " << eigs.eigenvalues() << std::endl;
        ...
        Eigen::DiagonalMatrix<double,Eigen::Dynamic> CCdiag = CC.asDiagonal();
        
        //std::cout << "XX rows: " << XX.rows() << ", XX cols: " << XX.cols() << ", CC len: " << CC.size() << ", DD rows: " << DD.rows() << ", DD cols: " << DD.cols();

        SpMat matToSolve(XX);
        matToSolve += rho * CCdiag;
        matToSolve += rho * DD;
        
        matToSolve.diagonal().array() += rho * 1e-6;
    ...
    void init_warm(double lambda_, double lambdaF_)
    {
        ...
        matToSolve.diagonal().array() += 1e-6;
...
#endif // ADMMOGLASSOFUSEDTALLSPARSE_H
