...
#include "ADMMogLassoFusedTallSparse.h"
                                         ...
                                         SEXP opts_)
{
    ...
    return List::create(Named("lambda")                 = lambda,
                        ...
                        Named("penalty.factor.fused")   = penalty_factor_fused,
...
RcppExport SEXP admm_oglasso_fused_sparse(SEXP x_,
                                          SEXP y_,
                                          SEXP delta_,
                                          SEXP group_,
                                          SEXP fused_,
                                          ...
                                          SEXP opts_)
{
    ...
    const MSpMat datX(as<MSpMat>(x_));
    Rcpp::NumericVector yy(y_);
    Rcpp::NumericVector ddelta(delta_);
    ...
    if (family(0) != "coxph") // family(0) != "gaussian" &
    {
        ...
        {
            fullbetamat = true;
            //add = 1; // dont need this, already accounted for in p
        }
    ...
    if (adaptive_lasso || adaptive_fused)
    {
        ...
        if(n > p)
        {
            if (p <= n)
            {
                ...
                if (maxbetaval > 15)
                {
                    solver_tall_2 = new ADMMogLassoFusedTallSparse(datX, datY, C, fused, n, p, M, intercept_bin,
                                                             ngroups, nfused,
                                                             family,
                                                             group_weights,
                                                             adaptive_fused_weights,
    ...
    for (int j = 0; j < nlambda_fused; j++)
    {
        ...
        //std::cout << "lambda fused: " << j << " " << ilambdaF << std::endl;
