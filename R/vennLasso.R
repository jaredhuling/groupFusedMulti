...
#'
#' @useDynLib vennLassoFused, .registration=TRUE
#'
#' @import Rcpp
#'
#' @export
                      ...
                      ...)
{
    ...
    direct.above.idx.list <- above.idx.list <- above.idx.full.list <- vector(mode = "list", length = M)
    ...
    above.idx.full.list <- above.idx.list
    direct.above.idx.list.withoutnone <- direct.above.idx.list
    if (any(rsc == 0)) 
    {
        above.idx.list[[which(rsc == 0)]] <- which(rsc == 0)
        direct.above.idx.list.withoutnone[[which(rsc == 0)]] <- which(rsc == 0)
    }
    ...
    group.mat <- Matrix::Matrix(0, nrow = (p * M), ncol = (p * M))
    ...
    if (fuse.none.group)
    {
        above.not.incl.idx.list <- lapply(1:length(direct.above.idx.list), 
                                          function(i) direct.above.idx.list[[i]][direct.above.idx.list[[i]] != i])
    } else
    {
        above.not.incl.idx.list <- lapply(1:length(direct.above.idx.list.withoutnone), 
                                          function(i) direct.above.idx.list.withoutnone[[i]][direct.above.idx.list.withoutnone[[i]] != i])
    }
    
    above.not.incl.idx.list <- lapply(above.not.incl.idx.list, function(el) if (is.null(el)){integer(0)} else {el})
    ...
    if (!is.null(lambda.fused))
    {
        ...
        fused.mat <- Matrix::Matrix(0, nrow = (p * N.fused), ncol = (p * M))
    ...
    fit <- vennLassoFused::oglasso(x                    = big.x,
                                   y                    = y,
                                   delta                = delta,
                                   group                = group.mat,
                                   fused                = fused.mat,
    ...
    if (family == "coxph")
    {
        if (fit_fused)
        {
            w <- vector(mode = "list", length = length(lambda.fused))
            for (lf in 1:length(lambda.fused))
            {
                yorder <- order(y)
                yy     <- y[yorder]
                deltaorder <- delta[yorder]
                w[[lf]]          <- array(0,c(N, nlambda))
                for (l in 1:nlambda)
                {
                    w[[lf]][,l] <- exp(big.x[yorder,] %*% fit$beta[[lf]][-1,l])
                }
            }
        } else
        {
    ...
    if (is.matrix(length(fit$lambda)))
    {
        nlm <- nrow(fit$lambda)
    } else
    {
        nlm <- length(fit$lambda)
    }
    
    n.free.param <- rep(NA, nlm)
    ...
    if (compute.se)
    {
        if (fit_fused)
        {
            stop("se computation not available for fused yet")
        }
 ...
 #                   w.refit[,l] <- exp(big.x[yorder,] %*% coef(re.fit))
        ...
        if (fit_fused)
        {
            for (lf in 1:length(lambda.fused))
            {
                for (l in 1:nlambda)
                {
                    if (intercept)
                    {
                        beta.cur.tmp <- beta.tmp[[lf]][1, l]
                        
                        grp.norms <- as.vector(sqrt( group.mat %*% beta.tmp[[lf]][-1, l] ^ 2 ))
    ...
    if (fit_fused)
    {
        beta.array <- intercept.list <- vector(mode = "list", length = length(lambda.fused))
        for (lf in 1:length(lambda.fused))
        {
            beta.array.tmp <- array(NA, dim = c(M, p, nlambda))
            if (compute.se)
            {
                stop("se computation not ready for fused lasso")
            ...
            for (c in 1:M) 
            {
                cur.var.idx <- which(which.not.missing.idx.combin[,c]==1)
                for (l in 1:nlambda) {
                    beta.array.tmp[c, cur.var.idx, l] <- beta.tmp[[lf]][1+(col.idx.vec[c]+1):col.idx.vec[c+1],l]
                }
            }
            intercept.list[[lf]] <- beta.tmp[[lf]][1,]
            combin.names <- apply(combin.mat, 1, function(x) paste(x, collapse = ","))
            dimnames(beta.array.tmp) <- list(combin.names, vnames, paste(seq(along = fit$lambda[,lf])))
    ...
    if (is.list(fit$df.strata))
    {
        free.params                <- lapply(fit$df.strata, function(lfs) colSums(lfs) + 1 * intercept)
        fit$df                     <- lapply(free.params, function(fp) N - fp)
        
        fit$aic <- fit$bic <- fit$mbic <- fit$aicc <- vector(mode = "list", length = length(fit$df))
        
        fit$logLik                 <- logLik.vennLasso(fit)
        
        c <- 4
        for (lf in 1:length(fit$logLik))
        {
            fit$aic[[lf]] <-      2 * free.params[[lf]] - 2 * fit$logLik[[lf]]
            fit$bic[[lf]] <- log(N) * free.params[[lf]] - 2 * fit$logLik[[lf]]
            fit$mbic[[lf]] <- fit$bic[[lf]] + 2 * free.params[[lf]] * log( (ncol(big.x) + 1 * intercept) / c - 1 )
            fit$aicc[[lf]] <- fit$aic[[lf]] + (2 * free.params[[lf]]) * (free.params[[lf]] + 1) / (N - free.params[[lf]] - 1)
        }
