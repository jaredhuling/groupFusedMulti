                           ...
                           ...)
{
    if (is.list(x$beta))
    {
        num.condition.combinations <- dim(x$beta[[1]])[1]
    ...
    if (is.list(x$beta))
    {
        nbeta <- as.matrix(x$beta[[lam.fused.idx]][which.subpop,-1,])
    } else
    {
        nbeta <- as.matrix(x$beta[which.subpop,-1,])
    }
    ...
    switch(xvar,
           ...
           "lambda" = {
               index <- if(is.list(x$beta)){x$lambda[,lam.fused.idx]} else {x$lambda}
               iname <- "Lambda"
               xlim <- rev(range(index))
           },
           "loglambda" = {
               index <- if(is.list(x$beta)){log(x$lambda[,lam.fused.idx])} else {log(x$lambda)}
               iname <- "Log Lambda"
               xlim <- rev(range(index))
           },
           "dev" = {
    ...
    if (is.list(x$beta))
    {
        rn <- rownames(x$beta[[1]])
        if (!is.null(rn))
        {
            main.txt <- paste0(rn[which.subpop], " LamFused=", round(x$lambda.fused[[lam.fused.idx]], 5))
        } else
        {
