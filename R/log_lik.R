...
logLik.vennLasso <- function(object, ...) 
{
    ...
    if (is.list(object$loss))
    {
        logL <- vector(mode = "list", length = length(object$loss))
        
        for (lf in 1:length(object$loss))
        {
            if (object$family == "gaussian")
            {
                
                resid.ss <- object$loss[[lf]]
                logL[[lf]] <- -0.5 * n * (log(2 * pi) - log(n) + log(resid.ss)) - 0.5 * n
            } else if (object$family == "binomial")
            {
                logL[[lf]] <- -1 * object$loss[[lf]]
