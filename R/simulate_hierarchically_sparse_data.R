...
#' @param effect.size.similarity How should the effect sizes of a variable across the subpopulations be related? \code{"sign"} for
                              ...
                              )
{
    ...
    if (is.null(beta)) 
    {
        ...
        beta.mat <- genHierSparseBeta(ncats, n.nzvars, hier.sparsity.param, effect.size.similarity,
                              ...
                              misspecification.prop = 0)
{
    ...
    if (effect.size.similarity == "none")
    {
    ...
    } else if (effect.size.similarity == "sign")
    {
      beta.mat <- matrix(runif(ncmb * n.nzvars, min=effect.size.max/2, max=effect.size.max), ncol = n.nzvars)
      
      for (v in 1:n.nzvars)
      {
        beta.mat[,v] <- beta.mat[,v] * (2 * rbinom(1, 1, 0.5) - 1)
      }
      
    } else if (effect.size.similarity == "mean")
    {
      for (v in 1:n.nzvars)
      {
        beta.mat[,v] <- runif(1, min=effect.size.max * 0.5, max=effect.size.max) * 
          (2 * rbinom(1, 1, 0.5) - 1)
        beta.mat[,v] <- beta.mat[,v] + runif(ncmb, min = -0.1, max = 0.1)
      }
    }
                                 ...
                                 effect.size.max, misspecification.prop)
{
    ...
    old_beta_no_cond <- beta.mat[id.no.conditions,]
    ...
    beta.mat[id.no.conditions,] <- old_beta_no_cond *
