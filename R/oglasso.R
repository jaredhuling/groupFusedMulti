...
#' n.vars <- 15
#'
#' true.beta <- c(rep(0,2), 1, -1, rep(0, 8), 0.5, -0.5, 1)
#'
                    ...
                    irls.maxit           = 100L)
{
    ...
    if (!is.null(group.weights)) {
        ...
        if (adaptive.lasso)
        {
            ...
            group.weights <- rep(1, ncol(group)) # sqrt(Matrix::colSums(group))
        } else
        {
    ...
    if (is.null(lambda)) 
    {
        ...
        {
            lambda <- matrix(0, nrow = 0, ncol = 1)
        } else
        {
            lambda <- matrix(0, nrow = 0, ncol = length(lambda.fused))
        }
    } else 
    {
        if (any(lambda < 0)) 
    ...
    if (length(lambda.fused) == 1 && lambda.fused == 0)
    {
    ...
    } else
    {
        if (use.alpha.param)
        {
            if (any(lambda.fused <= 0)) 
            {
                stop("lambda.fused should all be strictly between 0 and 1 if use.alpha.param = TRUE")
            }
            
            if (any(lambda.fused >= 1)) 
            {
                stop("lambda.fused should all be strictly between 0 and 1 if use.alpha.param = TRUE")
            }
            lambda.fused <- as.double(sort(lambda.fused))
        } else 
        {
            if (any(lambda < 0)) 
            {
    ...
    fit <- oglasso.fit(family,
                       is.sparse,
                       x=if (is.sparse & intercept) {cbind(1,x)} else {x}, 
                       y,
                       delta, group,
                       nlambda, lambda,
                       lambda.min.ratio,
                        ...
                        opts)
{
    ...
    {
        fused <- as(fused, "CsparseMatrix")
        fused <- as(fused, "dgCMatrix")
        
        if (is.sparse) {
            fit <- .Call("admm_oglasso_fused_sparse",
                         x_                    = x,
                         y_                    = y,
                         delta_                = delta,
                         group_                = group,
