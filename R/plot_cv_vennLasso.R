...
#' plot(fit.adapt) 
#' 
plot.cv.vennLasso <- function(x, sign.lambda = 1, best.fused = FALSE, ...) 
{
    ...
    if (!is.null(x$lambda.fused))
    {
        if (best.fused)
        {
            ...
            nzero <- apply(x$vennLasso.fit$beta[[x$which.min.fused]][,-1,], 3, function(bb) sum(bb != 0))
        ...
        } else
        {
            ...
            if (is.matrix(x$lambda))
            {
                if (x$vennLasso.fit$use.alpha.param)
                {
