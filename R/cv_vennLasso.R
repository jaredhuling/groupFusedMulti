                         ...
                         ...)
{
  ...
  if(missing(foldid)) 
  {
    ...
    ## here we take a stratified random sample within each subpopulation
  ...
  if (is.list(cvm))
  {
    names(cvm) <- names(cvsd) <- vennLasso.object$lambda.fused
    out=list(lambda        = vennLasso.object$lambda,
             lambda.fused  = vennLasso.object$lambda.fused,
             cvm           = cvm,
             cvsd          = cvsd,
    ...
    if (type.measure == "auc")
    {
      ...
      if (is.matrix(vennLasso.object$lambda))
      {
        lamin <- c(getmin(vennLasso.object$lambda[,lam.fused.min.idx], -cvm[[lam.fused.min.idx]], cvsd[[lam.fused.min.idx]]),
                   lambda.fused.min = vennLasso.object$lambda.fused[lam.fused.min.idx],
                   which.min.fused = lam.fused.min.idx)
      } else
      {
        lamin <- c(getmin(vennLasso.object$lambda, -cvm[[lam.fused.min.idx]], cvsd[[lam.fused.min.idx]]),
                   lambda.fused.min = vennLasso.object$lambda.fused[lam.fused.min.idx],
                   which.min.fused = lam.fused.min.idx)
      }
    } else
...
cv.venngaussian <- function(outlist,lambda,lambda.fused,x,y,groups,foldid,type.measure,grouped,keep=FALSE){
  ...
  if (!is.null(lambda.fused))
  {
    ...
    for (lf in 1:length(lambda.fused))
    {
      if (is.matrix(lambda))
      {
        predmat.tmp <- matrix(NA,length(y),NROW(lambda))
      } else
      {
        predmat.tmp <- matrix(NA,length(y),length(lambda))
      }
      nlams   <- double(nfolds)
      for(i in seq(nfolds))
      {
  ...
  } else
  {
    if (is.matrix(lambda))
    {
      predmat <- matrix(NA,length(y),NROW(lambda))
    } else
    {
  ...
  if (is.list(predmat))
  {
    ...
    for (lf in 1:length(predmat))
    {
      ...
      cvraw[[lf]] <- switch(type.measure,
                      "mse"      =(y - predmat[[lf]]) ^ 2,
                      "deviance" =(y - predmat[[lf]]) ^ 2,
                      "mae"      =abs(y - predmat[[lf]])
      )
      ...
      if(grouped)
      {
        cvob[[lf]]    <- cvcompute(cvraw[[lf]], rep(1, length(y)), foldid, nlams)
        cvraw[[lf]]   <- cvob[[lf]]$cvraw
      ...
      #cvm=apply(cvraw,2,weighted.mean,w=weights,na.rm=TRUE)
      cvm[[lf]]  <- apply(cvraw[[lf]], 2, mean, na.rm = TRUE)
